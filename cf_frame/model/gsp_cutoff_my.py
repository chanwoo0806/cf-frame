...
class GSP_Cutoff_MY(BaseModel):
    ...
    def set_filter(self):
        ...
        u = np.load(f'./svd/{args.dataset}/u_full.npy')[:, :self.freq_num]
        s = np.load(f'./svd/{args.dataset}/s_full.npy')[:self.freq_num]
        v = np.load(f'./svd/{args.dataset}/v_full.npy')[:, :self.freq_num]
        ...
        elif self.filter == 'exponential':
            self.filter_weight = sp.diags(np.exp(self.hyp * (-x)))
        elif self.filter == 'harmonic':
            self.filter_weight = sp.diags(1/(1 + self.hyp * x))
        elif self.filter == 'evenpoly':
            evenpoly = self.a * ((1 - x) ** 2) + self.b * ((1 - x) ** 4) + self.c * ((1 - x) ** 6)
            self.filter_weight = sp.diags(evenpoly)
